...
class ModelBase(object):
    ...
    @declared_attr
    def __table_args__(cls: Base) -> tuple:  # pylint: disable=no-self-argument
    ...
    def save(self, flush: bool = True, session: Session | None = None) -> None:
        ...
        session.add(self)  # TODO: typing: if session is None, it doesn't have add
        if flush:
            session.flush()  # TODO: typing: nor flush

    def delete(self, flush: bool = True, session: Session | None = None) -> None:
        """Delete this object"""
        session.delete(self)  # TODO: typing: if session is None, it doesn't have delete
        if flush:
            session.flush()  # TODO: typing: if session is None, it doesn't have flush
...
class SoftModelBase(ModelBase):
    ...
    @declared_attr
    def __table_args__(cls: Base) -> tuple:
...
class SQLAKnowledgeSource(Base, SoftModelBase):
...
class SQLASourceData(Base, SoftModelBase):
...
class SQLAEmbeddingModel(Base, SoftModelBase):
...
class SQLALLM(Base, SoftModelBase):
    __tablename__ = "llm"
...
class SQLACitation(Base, SoftModelBase):
...
class SQLAMessageBase(Base, SoftModelBase):
